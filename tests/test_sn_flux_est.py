...
def test_find_t_sys_gain():
    ...
    for beg, p_ra, p_dec, metadata, exp_t_sys, exp_t_sys_error, exp_gain, exp_gain_err in test_cases:
...
def test_find_pulsar_w50():
    ...
    for psr, exp_w50, exp_w50_err in test_cases:
        ...
        assert_almost_equal(exp_w50, w50, decmial=6)
...
def test_est_pulsar_flux():
    ...
    i    flux, flux_err = snfe.est_pulsar_flux(psr, obsid=obsid, f_mean=f_mean)
        assert_almost_equal(exp_flux, flux, deecimal=6)
...
def test_est_pulsar_sn():
    ...
    for psr, obsid, beg, end, ra, dec, exp_sn, exp_sn_err in test_cases:
        sn, sn_err = snfe.est_pulsar_sn(psr, obsid, beg=beg, end=end, p_ra=ra, d_dec=dec)
