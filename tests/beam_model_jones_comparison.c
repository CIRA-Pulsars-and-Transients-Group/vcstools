...
int main(int argc, char **argv)
{
    ...
    for beam_model in FEE2016, ANALYTIC
    int j
    for (j=0; j<2; j++){
        if (j==0)
        ...
        for (file_no = 0; file_no < nfiles; file_no++)
        {
            ...
            char comp_fname = (char*)malloc(sizeof(beam_model) + sizeof(opts->comp_fname) + 4*sizeof(char));
            sprintf(comp_fname, "%s_%s.dat", opts->comp_fname, beam_model);
            int ant, ch, pol
            FILE *f = fopen(fname, "w")
            for (ant = 0; ant < nstation; ant++){
                fprintf(f, "Antenna %3d\n", ant);
