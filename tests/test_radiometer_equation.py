...
J1820_0427_data = [0.00808072162938938, 11.269025292925557, 2786.22, 30.853343416881124, 4920.0, 597.3405435783852, 74.46134270114311, 194.85305766037393, 16.970470920838327, 570.440121819025, 91.48555161946545]
...
J1825_0935_data = [0.0031191764012526266, 3.0603445862819347, 2785.346666666667, 31.001558383768856, 4920.0, 440.5273973148081, 152.50926613148886, 254.19451614153593, 28.920583710253652, 435.22926553433194, 95.34158955673753]
...
J1834_0010_data  = [0.07982397127335845, 13.729849955263283, 2785.933333333333, 30.628397861389406, 4920.0, 23.191788224932292, 6.425243534096542, 22.00370081521057, 2.9503024248548564, 21.912687771229045, 88.4413267593695]
pulsars_to_test.append( ("J1834-0010", 107.99, J1834_0010_profile, J1834_0010_profile_bool, J1834_0010_data) )
# J1834-0426 259.982 mJy
...
J1834_0426_data = [0.022160061202548608, 15.090857513599753, 2785.64, 30.866503959254295, 4920.0, 285.84815663819836, 32.8456319671156, 83.58691261851845, 7.686412047058165, 268.4695072003402, 88.992666393354]
pulsars_to_test.append( ("J1834-0426", 259.982, J1834_0426_profile, J1834_0426_profile_bool, J1834_0426_data) )
# J1849-0636  84.704 mJy
...
J1849_0636_data = [0.09557737284098483, 21.981503499225628, 2786.0733333333337, 31.176548166138474, 4920.0, 140.55253403663716, 18.47486834457526, 24.14166262028616, 2.81938370753589, 127.91589552574786, 88.46042913038009]
pulsars_to_test.append( ("J1849-0636", 84.704, J1849_0636_profile, J1849_0636_profile_bool, J1849_0636_data) )
#---------------------------------------------------------------------------------------------------------------------
def test_flux_calc_radiometer_equation():
    for sn_method in ["eqn_7.1", "simple"]:
        for pulsar, imaging_flux, profile, profile_bool, data in pulsars_to_test:
            noise_std, w_equiv_bins, sefd, u_sefd, t_int, exp_e_S_mean,  exp_e_u_S_mean, exp_s_S_mean, exp_s_u_S_mean, _, _ = data
            S_mean, u_S_mean = flux_calc_radiometer_equation(profile, profile_bool,
                                        noise_std, w_equiv_bins,
                                        sefd, u_sefd, t_int, sn_method=sn_method)
            #print(pulsar, sn_method, imaging_flux, S_mean, u_S_mean)
            print(f"{pulsar} {sn_method:7s} {imaging_flux:6.1f} {S_mean:6.1f} {u_S_mean:6.1f}")
            if sn_method == "eqn_7.1":
                assert_almost_equal(S_mean,   exp_e_S_mean,   decimal=2)
                assert_almost_equal(u_S_mean, exp_e_u_S_mean, decimal=2)
            else:
                assert_almost_equal(S_mean,   exp_s_S_mean,   decimal=2)
                assert_almost_equal(u_S_mean, exp_s_u_S_mean, decimal=2)

def test_flux_calc_flux_profile():
    for pulsar, imaging_flux, profile, _, data in pulsars_to_test:
        noise_std, _, sefd, u_sefd, t_int, _,  _, _, _, exp_S_mean, exp_u_S_mean = data
        S_mean, u_S_mean = flux_calc_flux_profile(profile, noise_std,
                                                  sefd, u_sefd, t_int)
        print(f"{pulsar} {imaging_flux:6.1f} {S_mean:6.1f} {u_S_mean:6.1f} {(imaging_flux-S_mean)/u_S_mean:.2f}")
        #print(f"{pulsar} {imaging_flux:6.1f} {S_mean} {u_S_mean} {(imaging_flux-S_mean)/u_S_mean:.2f}")
