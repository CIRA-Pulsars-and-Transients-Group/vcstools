...
\begin{figure}[!p]
    \centering
    \includegraphics[scale=0.4,angle=270]{ipfb/ipfb-diagram1.pdf}
...
\begin{itemize}
    ...
    \item \texttt{int \blue{fcontext.nsamples} = ntap\_per\_call * fcontext.ntaps = \blue{3072}} (line 2250 of \texttt{make\_beam.c})
...
\begin{enumerate}
    ...
    \item Fourier transform (forward) each row of \vrbl{in} (output stored in \vrbl{out}).
...
\red{According to Steve, this algorithm appears to lack a necessary phase rotation step.}
...
\begin{itemize}
    \item \texttt{int \blue{nsamples} = fcontext.nsamples / nchan = \blue{24}}
    \item \texttt{int \blue{nchan\_in} = \blue{128}}
    \item \texttt{int \blue{mem\_size} = sizeof(Complex) * nsamples * nchan\_in * nchan\_in = \blue{393216} $\times$ sizeof(Complex)}
    \item \texttt{int \blue{nelements} = nsamples * nchan\_in = \blue{3072}}
\end{itemize}

\subsubsection*{Arrays:}

\begin{itemize}
    \item \texttt{Complex \blue{d\_buffer}\arrsize{nelements = \blue{3072}}}
    \item \texttt{Complex \blue{d\_signal}\arrsize{nsamples * nchan\_in * nchan\_in = \blue{393216}}}
    \item \texttt{Complex \blue{d\_filter\_kernel}\arrsize{nelements = \blue{3072}}}
\end{itemize}

\subsubsection*{Pseudocode:}

\begin{enumerate}
    \item Copy \vrbl{input} data into \vrbl{d\_buffer}.
    \item Copy the filter coefficients into \vrbl{d\_filter\_kernel}.
    \item Fill \vrbl{d\_signal} with zeros.
    \item Upsample \vrbl{d\_buffer} and store the result in \vrbl{d\_signal}.
    \item FFT (forward) \vrbl{d\_signal} (in-place).
    \item FFT (forward) \vrbl{d\_filter\_kernel} (in-place).
    \item Element-wise multiply \vrbl{d\_signal} and \vrbl{d\_filter\_kernel}.
        The two arrays are different sizes, and the implementation looks as though the function is called with a parameter that lets you choose what position within \vrbl{d\_signal} you want to apply the filter at.
        \red{I'm not convinced that this is implemented correctly.}
    \item FFT (backward) \vrbl{d\_signal} (in-place)
    \item Shift and reduce: Add the ``rows'' of \vrbl{d\_signal} (still representing channelised data) together, making sure to shift each row by an appropriate amount (which turns out to be a multiple of 24 samples in this case).
        The result of the reduction is stored in the last ``row'' of \vrbl{d\_signal}.
        \red{This part of the code is currently commented out.}
    \item The desired output is the last ``row'' of \vrbl{d\_signal}.
\end{enumerate}
