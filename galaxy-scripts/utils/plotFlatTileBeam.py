...
def plot_beam_pattern(obsid, obsfreq, obstime, ra, dec, cutoff=0.1):
	...
	location = EarthLocation(lat=-26.7033*u.deg, lon=116.671*u.deg, height=377.827*u.m)	
	...
	axtxt.axis('off')
	axtxt.text(0.01,0.5,"Obs ID: {0}\nFrequency: {1:.2f}MHz\nBeam Pmax: {2:.3f}\nBeam half-Pmax: {3:.3f}".format(obsid,obsfreq/1.e6,pmax,hpp),verticalalignment='center')
	...
	axP.scatter(ptAZ, ptZA, marker="+", s=20, color='magenta') # plot the tile beam pointing
	for ta,tz,c in zip(targetAZ, targetZA, colours):
		axP.scatter(ta, tz, marker="x", s=20, color=c) # plot the target track through the beam
	axP.set_xlim(0,360)
	axP.set_ylim(0,90)
