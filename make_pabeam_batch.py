...
def write_batch_files(tmin,tmax,step,thetares,phires,nnodes,ra,dec,obsid,freq,eff,flaggedtiles,write,odir):
	...
	for i in range(len(times)-1):
		fname = "make_pabeam_{0}_{1}MHz.batch".format(times[i],freq/1e6)
		onamebase = "{0}_{1}_{2}MHz_{3}_{4}".format(obsid,float(times[i]),freq/1e6,ra,dec)
	...
	fname = "make_pabeam_{0}_{1}MHz.batch".format(times[-1],freq/1e6)
	onamebase = "{0}_{1}_{2}MHz_{3}_{4}".format(obsid,float(times[-1]),freq/1e6,ra,dec)
	with open(fname,'w') as f:
		f.write("#!/bin/bash -l\n\n")
		f.write("#SBATCH --account=mwaops\n#SBATCH --partition=workq\n#SBATCH --nodes={0}\n".format(nnodes))
		f.write("#SBATCH --time=12:00:00\n#SBATCH --output={0}\n\n".format(fname.replace(".batch",".out")))
...
parser.add_argument("--nodes",type=int,help="Number of nodes to use per run (more than 10 is overkill and will actually be slower...) [default: 5]",default=5)
