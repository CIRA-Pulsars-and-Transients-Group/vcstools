...
void usage() {
    ...
    fprintf(stderr,"-g <miriad complex gains> -- complex gains from MIRIAD\n");
...
int main(int argc, char **argv) {
    ...
    if (procdirroot == NULL) {
        ...
        // Generate the directory name from root string + processor number
        dir_index = me+1;
        sprintf(procdir,"%s%02d",procdirroot,dir_index);
        fprintf(stdout,"Looking for data in %s\n",procdir);
        // Glob .dat files in the specified subdirectory
        char pattern[256];
        sprintf(pattern, "%s/*.%s", procdir,extn);
