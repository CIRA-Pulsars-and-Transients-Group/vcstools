...
if rank == 0:
    ...
    with open(oname.replace(".dat", ".stats"), "w") as f:
        f.write("==== Summary ====")
        f.write("** Pointing **")
        f.write("(RA, Dec)      : {0} {1}".format(ra, dec))
        f.write("(Az, ZA) [rad] : {0:.3f} {1:.3f}".format(targetAZ, targetZA))
        f.write("(Az, ZA) [deg] : {0:.3f} {1:.3f}".format(targetAZdeg, targetZAdeg))
        f.write("\n")
        f.write("** Time **")
        f.write("GPS : {0}".format(time.gps))
        f.write("UTC : {0}".format(time.utc))
        f.write("MJD : {0}".format(time.mjd))
        f.write("\n")
        f.write("** Telescope parameters **")
        f.write("Observation ID       : {0}".format(args.obsid))
        f.write("Frequency            : {0} MHz".format(args.freq/1e6))
        f.write("Radiation efficiency : {0}".format(args.efficiency))
        f.write("Flagged tiles        : {0}".format(" ".join(flags)))
        f.write("\n")
        f.write("** Simulation resolution **")
        f.write("Theta (ZA) resolution    [deg] : {0}".format(tres))
        f.write("                      [arcmin] : {0}".format(tres * 60))
        f.write("Phi   (Az) resolution    [deg] : {0}".format(pres))
        f.write("                      [arcmin] : {0}".format(pres * 60))
        f.write("\n")
        f.write("** Calculated quantities **")
        f.write("Beam solid angle [steradians] : {0}".format(result))
        f.write("Beam solid angle    [sq. deg] : {0}".format(np.degrees(np.degrees(result))))
        f.write("Effective area          [m^2] : {0:.3f}".format(eff_area))
