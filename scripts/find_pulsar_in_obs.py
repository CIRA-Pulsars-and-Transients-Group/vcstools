...
This code is used to list the sources within the beam of observations IDs or
(using --obs_for_source) to list all the observations for each source. The sources
can be input serval ways: using a list of pulsar names (--pulsar), using a
complete catalogue file of pulsars (--dl_PSRCAT) or RRATs (--RRAT and --dl_RRAT),
using a compatable catalogue (--in_cat with the help of --names and --coordstype)
or using a RA and DEC coordinate (--coords). The observation IDs can be input
(--obsid) or gathered from a directory (--FITS_dir). The default is to search
all observation IDs from http://mwa-metadata01.pawsey.org.au/metadata/ that have
voltages and list every known pulsar from PSRCAT in each observation ID.

The two most comon uses for this code is to search a single observation ID for
pulsars, like so:
...
def parse_input():
    desc = 'This code is used to list the sources within the beam of ' + \
    'observations IDs or (using --obs_for_source) to list all the observations ' + \
    'for each source. The sources can be input serval ways: using a list of ' + \
    'pulsar names (--pulsar), using a complete catalogue file of pulsars ' + \
    '(--dl_PSRCAT) or RRATs (--RRAT and --dl_RRAT), using a compatable ' + \
    'catalogue (--in_cat with the help of --names and --coordstype) or using ' + \
    'a RA and DEC coordinate (--coords). The observation IDs can be input ' + \
    '(--obsid) or gathered from a directory (--FITS_dir). The default is to ' + \
    'search all observation IDs from http://mwa-metadata01.pawsey.org.au/metadata/ ' + \
    'that have voltages and list every known pulsar from PSRCAT in each observation ID.'
    ...
    parser.add_argument('-L', '--loglvl', type=str, choices=loglevels.keys(), default='INFO',
        help='Logger verbosity level. [default: %(default)s]')
    parser.add_argument('--output', type=str, default = './',
        help='Chooses a directory for all the text files to be output to. The default ' + \
    ...
    # Source options
    sourargs = parser.add_argument_group('Source options',
        'The different options to control which sources are used. Default is all ' + \
        'known pulsars.')
    sourargs.add_argument('-p', '--pulsar', type=str, nargs='*', default = None,
        help='Searches for all known pulsars. This is the default. To search for ' + \
        'individual pulsars list their Jnames in the format "-p J0534+2200 J0630-2834".')
    sourargs.add_argument('--max_dm', type=float, default = 250.,
        help='The maximum DM for pulsars. All pulsars with DMs higher than the ' + \
        'maximum will not be included in output files. [default: %(default)s]')
    sourargs.add_argument('--source_type', type=str, default='Pulsar',
        help="An astronomical source type from ['Pulsar', 'FRB', 'rFRB', 'GC', " + \
        "'RRATs', Fermi] to search for all sources in their respective web catalogue.")
    sourargs.add_argument('--in_cat', type=str,
        help='Location of source catalogue, must be a csv where each line is in ' + \
        'the format "source_name, hh:mm:ss.ss, +dd:mm:ss.ss".')
    # TODO: make coord input more robust to include input as sex or deg and perhaps other coordinte systmes
    sourargs.add_argument('-c', '--coords', type=str, nargs='*',
        help='String containing the source\'s coordinates to be searched for in ' + \
        'the format "RA_DEC" "RA_DEC". Must be enterered as either: ' + \
        '"hh:mm:ss.ss_+dd:mm:ss.ss" or "deg_-deg". Please only use one format.')

    # Observation options
    obargs = parser.add_argument_group('Observation ID options',
    ...
    obargs.add_argument('--all_volt', action='store_true',
        ...
        'archive. Some incoherent observation ID files may be archived even ' + \
        'though there are raw voltage files. The default is to only include ' + \
        'files with raw voltage files.')
    obargs.add_argument('--cal_check', action='store_true',
        help='Check the MWA Pulsar Database to check if the obsid has every ' + \
        'succesfully detected a pulsar and if it has a calibration solution.')
    ...
    if args.in_cat and args.coords:
        logger.error("Can't use both --in_cat and --coords. Exiting.")
        exit(1)
    if args.obsid and args.FITS_dir:
        logger.error("Can't use both --obsid and --FITS_dir. Exiting.")
        exit(1)
    if args.beam not in beam_models:
        logger.error(f'Unknown beam model. Please use one of {beam_models}. Exiting.')
        exit(1)
...
def main():
    args, logger = parse_input()

    logger.info('Gathering sources...')
    degrees_check = False
    if args.in_cat:
        # Read sources from CSV catalogue
        names_ra_dec = []
        ...
        # Read sources from command line arguments
        names_ra_dec = []
        ...
        # Format ra and dec if they are in sexigesimal format
        if not degrees_check:
            names_ra_dec = format_ra_dec(names_ra_dec, ra_col=1, dec_col=2)
    else:
        # Get sources from the catalogue
        # TODO: use psrqpy
        names_ra_dec = grab_source_alog(
        ...
        if len(names_ra_dec) == 0:
            raise NoSourcesError(
                f"""No sources found in catalogue with:
                    source type:    {args.source_type}
                    pulsars:        {args.pulsar}
                    max dm:         {args.max_dm}"""
            )

    # Convert source list to a numpy array
    names_ra_dec = np.array(names_ra_dec)

    # Check if the user wants to use --obs_for_source
    if (len(names_ra_dec) == 1) and (not args.obs_for_source):
        args.obs_for_source = yes_no(
            'You are only searching for one pulsar so it is recommened that you ' + \
            'use --obs_for_source. Would you like to use --obs_for_source. (Y/n)'
        )
    ...
    logger.debug(f'{names_ra_dec=}')
    logger.debug(f'{obsid_list=}')
    logger.info('Getting observation metadata and calculating the tile beam...')
    output_data, obsid_meta = find_sources_in_obs(
        obsid_list,
        names_ra_dec,
        obs_for_source=args.obs_for_source,
        dt_input=dt,
        beam=args.beam,
        min_z_power=args.min_power,
        cal_check=args.cal_check,
        all_volt=args.all_volt,
    ...
    if args.obs_for_source:
        write_output_source_files(
            ...
            cal_check=args.cal_check,
            SN_est=args.sn_est,
            flux_est=args.flux_est,
            plot_est=args.plot_est,
