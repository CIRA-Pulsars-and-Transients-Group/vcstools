...
if [[ $HOSTNAME != garrawarla* ]]; then
    groupUsage=$(lfs quota -q -u $USER /group | head -n 1 | awk '{print $2/1073741824}')
    echo "  Used on /group: $(printf "%7.3f" "$groupUsage") TB"
fi
astroUsage=$(lfs quota -q -u $USER /astro | head -n 1 | awk '{print $2/1073741824}')
...
if [[ $HOSTNAME != garrawarla* ]]; then
    ...
    groupQuota=$(lfs quota -q -g mwavcs /group | head -n 1 | awk '{print $4/1073741824}')
    # Disk space used on /group for the mwavcs group, in terabytes
    groupUsage=$(lfs quota -q -g mwavcs /group | head -n 1 | awk '{print $2/1073741824}')
...
# Quotas for /astro
astroQuota=$(lfs quota -q -g mwavcs /astro | head -n 1 | awk '{print $4/1073741824}')
# Disk space used on /astro for the mwavcs group, in terabytes
astroUsage=$(lfs quota -q -g mwavcs /astro | head -n 1 | awk '{print $2/1073741824}')
# Percentage of quota filled
