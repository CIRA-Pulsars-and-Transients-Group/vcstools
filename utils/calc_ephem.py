...
"ASKAP": 15,
"GMRT": 17,
"VLA": 20, # limit for high frequnecies (S-band and above), can go to 10d for L and P-band
"Arecibo": 70,
"Effelsberg": 8,
"MeerKAT": 15
}
...
class Observatory(object):
    ...
    def __init__(self, name, latitude=None, longitude=None, elevation=None):
        ...
        if name in site_dict.keys():
            print "Found observatory coordinates for '{0}': {1}".format(name, site_dict[name])
            self.name = name
            ...
            print "Observatory '{0}' not found in internal list".format(name)
            if (latitude is None) or (longitude is None) or (elevation is None):
                ...
                print "Using provided name and location"
                print "\ttelescope = {0}".format(name)
                print "\tlon, lat, elev = ({0}, {1}, {2})".format(longitude, latitude, elevation)
                self.name = name
...
def plot_ephem(ax, times, obs, plot_sun=False, site=None):
    ...
    if obs.elev_limit is not None:
        ax.axhline(obs.elev_limit, ls=":", lw=3, color=eph[0].get_color(), label="{0} elev. limit".format(obs.name))
...
def calculate_ephem(ra, dec, date, tzoffset=0, site="MWA", show_sun=False, center=False):
    ...
    print "\nTarget: {0}\n".format(coords.to_string('hmsdms'))
    for s in site:
        o = Observatory(s)
        o.compute_target_position(coords, times, tz)
        print "\tmax. target elevation = {0}".format(o.altmax)
        print "\ttime of max. elevation = {0} UTC{1}".format(o.maxtimeLocalStr, o.utcoffsetStr)
        print

        if show_sun:
            o.compute_sun_position(times[::50])
        site_max.append(plot_ephem(ax, plttimes, o, plot_sun=show_sun, site=s))
